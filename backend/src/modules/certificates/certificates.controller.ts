...
const getQueryVariants = (input: string): string[] => {
...
export const verifyCertificate = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const searchVariants = getQueryVariants(serialNumber as string);
...
export const deleteCertificate = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const careerProfile = await prisma.careerProfile.findUnique({ where: { certificate_id: cert.id }, select: { id: true } });
    if (careerProfile) {
